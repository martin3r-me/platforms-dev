        ...
        <div class="d-flex items-center justify-between text-xs text-[var(--ui-muted)] mt-2 pt-2 border-t border-[var(--ui-border)]/20">
            <div class="d-flex items-center gap-1.5 min-w-0">
                ...
                @if($userInCharge)
                    ...
                    @else
                        <span class="inline-flex items-center justify-center w-4 h-4 rounded-full bg-[var(--ui-muted-5)] border border-[var(--ui-border)]/40 text-[9px] text-[var(--ui-muted)]">
            ...
            @if($issue->due_date)
                <span class="flex items-center gap-1 flex-shrink-0 {{ $issue->due_date->isPast() && !$isDone ? 'text-[var(--ui-danger)]' : '' }}">
